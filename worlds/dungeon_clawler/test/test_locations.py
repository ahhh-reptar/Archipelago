...
class TestAccessibilityRules(DungeonClawlerTestBase):
    ...
    def test_win_with_bunner(self):
        location_name = character_location_name(Character.chief_bunner)
        ...
        self.collect([self.get_item_by_name(CombatItem.dagger)] * 20)

        self.assertFalse(self.multiworld.state.can_reach_location(location_name, self.player))

        self.collect(self.get_item_by_name(CombatItem.small_shield))
        self.collect(self.get_item_by_name(CombatItem.spikey_shield))
        self.collect(self.get_item_by_name(CombatItem.big_shield))
        self.collect(self.get_item_by_name(CombatItem.tower_shield))
        self.collect(self.get_item_by_name(CombatItem.metal_shield))
