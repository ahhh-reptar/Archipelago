...
from .. import GAME_NAME, DungeonClawlerWorld
...
class DungeonClawlerTestBase(WorldTestBase):
    game = GAME_NAME
    world: DungeonClawlerWorld
...
def setup_dungeon_clawler_solo_multiworld_with_fill(test_options=None, seed=None, fill=False, _cache: Dict[FrozenSet[Tuple[str, Any]], MultiWorld] = {}) -> MultiWorld: #noqa
    ...
    multiworld = setup_base_solo_multiworld(DungeonClawlerWorld, (), seed=seed)
    # print(f"Seed: {multiworld.seed}") # Uncomment to print the seed for every test
    args = Namespace()
    for name, option in DungeonClawlerWorld.options_dataclass.type_hints.items():
