...
def set_entrance_rules(logic: StardewLogic, multi_world, player, world_options: StardewOptions):
    for floor in range(5, 120 + 5, 5):
        MultiWorldRules.set_rule(multi_world.get_entrance(dig_to_mines_floor(floor), player),
                                 logic.mine.can_mine_to_floor(floor).simplify())
    ...
    for floor in range(25, 200 + 25, 25):
        MultiWorldRules.set_rule(multi_world.get_entrance(dig_to_skull_floor(floor), player),
                                 logic.mine.can_mine_to_skull_cavern_floor(floor).simplify())
...
def set_help_wanted_fishing_rule(logic: StardewLogic, multi_world, player, month_rule, quest_number):
    ...
    fishing_rule = month_rule & logic.skill.can_fish()
...
def set_help_wanted_slay_monsters_rule(logic: StardewLogic, multi_world, player, month_rule, quest_number):
    ...
    slay_rule = month_rule & logic.combat.can_fight_at_level("Basic")
...
def set_museum_individual_donations_rules(all_location_names, logic: StardewLogic, multi_world, museum_prefix, player):
    ...
    for museum_location in all_donations:
        if museum_location.name in all_location_names:
            ...
            rule = logic.museum.can_donate_museum_item(all_museum_items_by_name[donation_name]) & logic.received("Traveling Merchant Metal Detector", required_detectors)
...
def set_museum_milestone_rule(logic: StardewLogic, multi_world: MultiWorld, museum_milestone, museum_prefix: str,
    ...
    if milestone_name.endswith(donations_suffix):
        rule = get_museum_item_count_rule(logic, donations_suffix, milestone_name, all_museum_items, logic.museum.can_donate_museum_items)
    elif milestone_name.endswith(minerals_suffix):
        rule = get_museum_item_count_rule(logic, minerals_suffix, milestone_name, all_museum_minerals, logic.museum.can_donate_museum_minerals)
    elif milestone_name.endswith(artifacts_suffix):
        rule = get_museum_item_count_rule(logic, artifacts_suffix, milestone_name, all_museum_artifacts, logic.museum.can_donate_museum_artifacts)
    elif milestone_name == "Dwarf Scrolls":
        rule = And([logic.museum.can_donate_museum_item(item) for item in dwarf_scrolls]) & logic.received(metal_detector, 4)
    elif milestone_name == "Skeleton Front":
        rule = And([logic.museum.can_donate_museum_item(item) for item in skeleton_front]) & logic.received(metal_detector, 4)
    elif milestone_name == "Skeleton Middle":
        rule = And([logic.museum.can_donate_museum_item(item) for item in skeleton_middle]) & logic.received(metal_detector, 4)
    elif milestone_name == "Skeleton Back":
        rule = And([logic.museum.can_donate_museum_item(item) for item in skeleton_back]) & logic.received(metal_detector, 4)
    elif milestone_name == "Ancient Seed":
        rule = logic.museum.can_donate_museum_item(Artifact.ancient_seed) & logic.received(metal_detector, 4)
    if rule is None:
        return
...
def set_magic_spell_rules(logic: StardewLogic, multi_world: MultiWorld, player: int, world_options: StardewOptions):
    ...
    MultiWorldRules.add_rule(multi_world.get_location("Analyze: Clear Debris", player),
                             ((logic.tool.has_tool(Tool.axe, ToolMaterial.basic) | logic.tool.has_tool(Tool.pickaxe, ToolMaterial.basic))
                              & logic.mod.magic.can_use_altar()).simplify())
    MultiWorldRules.add_rule(multi_world.get_location("Analyze: Till", player),
                             (logic.tool.has_tool(Tool.hoe, ToolMaterial.basic) & logic.mod.magic.can_use_altar()).simplify())
    MultiWorldRules.add_rule(multi_world.get_location("Analyze: Water", player),
                             (logic.tool.has_tool(Tool.watering_can, ToolMaterial.basic) & logic.mod.magic.can_use_altar()).simplify())
    MultiWorldRules.add_rule(multi_world.get_location("Analyze All Toil School Locations", player),
                             (logic.tool.has_tool(Tool.watering_can, ToolMaterial.basic) & logic.tool.has_tool(Tool.hoe, ToolMaterial.basic)
                              & (logic.tool.has_tool(Tool.axe, ToolMaterial.basic) | logic.tool.has_tool(Tool.pickaxe, ToolMaterial.basic))
                              & logic.mod.magic.can_use_altar()).simplify())
    ...
    MultiWorldRules.add_rule(multi_world.get_location("Analyze: Evac", player),
                             (logic.ability.can_mine_perfectly() & logic.mod.magic.can_use_altar()).simplify())
    MultiWorldRules.add_rule(multi_world.get_location("Analyze: Haste", player),
                             (logic.has("Coffee") & logic.mod.magic.can_use_altar()).simplify())
    MultiWorldRules.add_rule(multi_world.get_location("Analyze: Heal", player),
                             (logic.has("Life Elixir") & logic.mod.magic.can_use_altar()).simplify())
    ...
    MultiWorldRules.add_rule(multi_world.get_location("Analyze: Descend", player),
                             (logic.region.can_reach(Region.mines) & logic.mod.magic.can_use_altar()).simplify())
    ...
    MultiWorldRules.add_rule(multi_world.get_location("Analyze: Tendrils", player),
                             (logic.region.can_reach(Region.farm) & logic.mod.magic.can_use_altar()).simplify())
    MultiWorldRules.add_rule(multi_world.get_location("Analyze: Shockwave", player),
                             (logic.has("Earth Crystal") & logic.mod.magic.can_use_altar()).simplify())
    ...
    MultiWorldRules.add_rule(multi_world.get_location("Analyze: Lucksteal", player),
                             (logic.region.can_reach(Region.witch_hut) & logic.mod.magic.can_use_altar()).simplify())
    MultiWorldRules.add_rule(multi_world.get_location("Analyze: Bloodmana", player),
                             (logic.region.can_reach(Region.mines_floor_100) & logic.mod.magic.can_use_altar()).simplify())
