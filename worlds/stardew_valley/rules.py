...
def set_sve_rules(logic: StardewLogic, multiworld: MultiWorld, player: int, content: StardewContent):
    ...
    set_entrance_rule(multiworld, player, SVEEntrance.to_aurora_basement, logic.mod.quest.has_completed_aurora_vineyard_bundle())  #my entrance rules are apparently fubar and I don't get why, I basically just copied stuff already here
    #set_entrance_rule(multiworld, player, SVEEntrance.witch_warp_cave_to_forbidden_maze, logic.quest.has_dark_talisman() | (logic.mod.magic.can_blink()))  This isn't quite right but I need to tell archipelago there's a zone between the warp cave and the witch hut and I don't know how
    ##set_entrance_rule(multiworld, player, SVEEntrance.forbidden_maze_to_henchman_backyard, (logic.quest.can_complete.quest(Quest.goblin_problem) & logic.has("Aegis Elixir")) | (logic.quest.can_complete_quest(Quest.goblin_problem) & logic.combat.can_fight_at_level(Performance.maximum)))
    #set_entrance_rule(multiworld, player, SVEEntrance.forbidden_maze_to_witch_swamp, (logic.quest.can_complete.quest(Quest.goblin_problem) & logic.has("Aegis Elixir")) | (logic.quest.can_complete_quest(Quest.goblin_problem) & logic.combat.can_fight_at_level(Performance.maximum)))
    #set_entrance_rule(multiworld, player, SVEEntrance.highlands_to_diamond_cavern, (logic.tool.has_tool(Tool.pickaxe, ToolMaterial.iron) & logic.mod.sve.has_marlon_boat()) | logic.mod.sve.has_marlon_boat() & logic.mod.magic.can_blink())
