...
class MonsterLogic(BaseLogic[Union[MonsterLogicMixin, RegionLogicMixin, CombatLogicMixin, TimeLogicMixin, OptionLogicMixin]]):
    ...
    # Should be cached
    def can_kill_any(self, monsters: Union[Iterable[monster_data.StardewMonster], Hashable], amount_tier: int = 0) -> StardewRule:
        rules = [self.logic.monster.can_kill(monster, amount_tier) for monster in monsters]
        return Or(*rules)

    # Should be cached
    def can_kill_all(self, monsters: Union[Iterable[monster_data.StardewMonster], Hashable], amount_tier: int = 0) -> StardewRule:
        rules = [self.logic.monster.can_kill(monster, amount_tier) for monster in monsters]
        return And(*rules)
