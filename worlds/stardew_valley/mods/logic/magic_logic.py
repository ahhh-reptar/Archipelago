...
class MagicLogic(BaseLogic[Union[RegionLogicMixin, ReceivedLogicMixin, OptionLogicMixin]]):
    def can_use_clear_debris_instead_of_tool_level(self, level: int) -> StardewRule:
        return self.logic.option.contains_choice_or_false(options.Mods,
                                                          ...
                                                          match=(self.logic.received(MagicSpell.clear_debris) & self.can_use_altar() &
                                                                 self.logic.received(ModSkillLevel.magic_level, level)))

    def can_use_altar(self) -> StardewRule:
        return self.logic.option.contains_choice_or_false(options.Mods,
                                                          value=ModNames.magic,
                                                          match=self.logic.region.can_reach(MagicRegion.altar))

    def has_any_spell(self) -> StardewRule:
        return self.logic.option.contains_choice_or_false(options.Mods,
                                                          value=ModNames.magic,
                                                          match=self.can_use_altar())
    ...
    def has_decent_spells(self) -> StardewRule:
        ...
        return self.logic.option.contains_choice_or_false(options.Mods,
                                                          value=ModNames.magic,
                                                          match=magic_resource_rule & magic_attack_options_rule)
    ...
    def has_good_spells(self) -> StardewRule:
        ...
        return self.logic.option.contains_choice_or_false(options.Mods,
                                                          value=ModNames.magic,
                                                          match=magic_resource_rule & magic_attack_options_rule & magic_support_options_rule)
    ...
    def can_blink(self) -> StardewRule:
        return self.logic.option.contains_choice_or_false(options.Mods,
                                                          value=ModNames.magic,
                                                          match=self.logic.received(MagicSpell.blink) & self.can_use_altar())
